...
songplay_table_create = ("""CREATE TABLE IF NOT EXISTS songplays (songplay_id serial, start_time timestamp NOT NULL, user_id int NOT NULL, level varchar, song_id text, artist_id varchar, session_id int, location varchar, user_agent text, PRIMARY KEY (songplay_id, start_time), FOREIGN KEY (user_id) REFERENCES users(user_id), FOREIGN KEY (song_id) REFERENCES songs(song_id ), FOREIGN KEY (artist_id) REFERENCES artists(artist_id), FOREIGN KEY (start_time) REFERENCES time(start_time))""" )
...
song_select = (""" SELECT s.song_id songid, s.artist_id artistid FROM songs s  JOIN artists a on s.artist_id = a.artist_id WHERE s.title = %s AND a.name = %s AND s.duration = %s
